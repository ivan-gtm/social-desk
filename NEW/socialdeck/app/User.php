...
class User extends Model
{
	    ...
	    public function isAdmin(){
	    	if (isset($this->account_type) && in_array($this->account_type, array("developer", "admin"))) {
	    		return true;
	    	}
	    ...
	    public function _select($uniqid)
	    {
	     ...
	     * @return self
	     */
	    public function _extendDefaults()
	    {
	    	$defaults = array(
	    		"account_type" => "member",
	    		"email" => uniqid()."@thepostcode.co",
	    		"username" => "user_".uniqid(),
	    		"password" => uniqid(),
	    		"firstname" => "",
	    		"lastname" => "",
	    		"package_id" => "0",
	    		"package_subscription" => "0",
	    		"settings" => "{}",
	    		"preferences" => "{}",
	    		"is_active" => "0",
	    ...
	    {	
	    	if ($User != null) {
	    		if ( $this->account_type == "developer" || $this->id == $User->id ) {
    				return true;
    			}
    			...
    			if (
    				...
    				(
	    				in_array( $User->account_type, array("member", "admin") ) ||
	    ...
	    public function isExpired()
	    {
	    	...
	    	if (isset($this->expire_date)) {
	    		...
	    		$expiration_date = \Carbon\Carbon::parse($this->expire_date);
	     ...
	     * @return boolean 
	     */
	    public function _isEmailVerified()
	    {
	    	if (!$this->isAvailable()) {
	    		return false;
	    	}
	     ...
	     * @return [bool]                  
	     */
	    public function _sendVerificationEmail($force_new = false)
	    {
	    	if (!$this->isAvailable()) {
	    		return false;
	    	}
	     ...
	     */
	    public function _setEmailAsVerified()
	    {
	    	if (!$this->isAvailable()) {
	    		return false;
	    	}
