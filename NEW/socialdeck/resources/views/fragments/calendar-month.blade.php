    ...
    <div class="container-1200">
        <?php if (count($Accounts) > 0): ?>
            <div class="row clearfix pos-r">
                <form class="account-selector clearfix" action="<?= Config::get('constants.APPURL')."/calendar/".$year."/".$month ?>" method="GET">
                ...
                <div class="calendar-month-switch">
                    <?php 
                        ...
                        $date = \Carbon\Carbon::now($AuthUser->preferences->timezone)->setTimezone(date_default_timezone_get())->startOfMonth();
                    ?>

                    <div class="month">
                        <a class="sli sli-arrow-left nav left" href="<?= Config::get('constants.APPURL') . "/calendar/" . ($prevmonth == "12" ? $year - 1 : $year) . "/" . $prevmonth . (isset($ActiveAccount) ? "?account=".$ActiveAccount->id : ""); ?>" ></a>
                        <?= $date->format("F") ?>
                        <a class="sli sli-arrow-right nav right" href="<?= Config::get('constants.APPURL') . "/calendar/" . ($nextmonth == "01" ? $year + 1 : $year) . "/" . $nextmonth . (isset($ActiveAccount) ? "?account=".$ActiveAccount->id : ""); ?>" ></a>
                    </div>
            ...
            <div class="row clearfix">
                <div class="calendar">
                    ...
                    <?php 
                        ...
                        $now = \Carbon\Carbon::now(date_default_timezone_get())->setTimezone($AuthUser->preferences->timezone);
                        // $now = new DateTime("now", new DateTimeZone(date_default_timezone_get()));
                        // $now->setTimezone(new DateTimeZone($AuthUser->preferences->timezone));
                        // exit;
                    ?>
                    <div class="clearfix">
                        <?php if ($month_firstday_number > 1): ?>
                            <?php for ($i=1; $i < $month_firstday_number; $i++): ?>
                                <?php 
                                    ...
                                    $date = new DateTime($date, new DateTimeZone($AuthUser->preferences->timezone));
                                ?>
                                <div class='cell in-other-month'>
                                    <div class='cell-inner'>
                                        ...
                                        <a href="<?= Config::get('constants.APPURL') . "/calendar/" . $date->format("Y/m") ?>" class="full-link"></a>
                                    </div>
                                </div>
                            <?php endfor; ?>
                        <?php endif ?>
                            ...
                            <div class="cell <?= $date->format("Y-m-d") == $now->format("Y-m-d") ? "today" : "" ?>">
                                <div class='cell-inner'>
                                    ...
                                    <?php if ($date->format("Y-m-d") >= $now->format("Y-m-d")): ?>
                                        <a class="add-post" href="<?= Config::get('constants.APPURL')."/post?date=" . $date->format("Y-m-d") . (isset($ActiveAccount) ? "&account=".$ActiveAccount->id : "") ?>">
                                    ...
                                    <?php if (!empty($postcounts[$date->format("d")])): ?>
                                        ...
                                        <div class="count">
                                            ...
                                            <div class="intro">
                                                {{ $total }} posts
                                                <a href="<?= Config::get('constants.APPURL') . "/calendar/" . $date->format("Y/m/d") . (isset($ActiveAccount) ? "?account=".$ActiveAccount->id : "") ?>" class="full-link"></a>
                                            </div>
